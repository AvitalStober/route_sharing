...
const PopUpRoute: React.FC<PopUpRouteProps> = ({ onClose, routeId, filtered }) => {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full w-1/3 bg-white shadow-lg p-6 overflow-y-auto z-50 transition-transform duration-[500ms] transform ${isVisible ? "translate-x-0" : "translate-x-full"}`}
    >
      <button
        onClick={handleClose}
        className="absolute top-4 right-4 bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600"
